---
title: "Brilliant Biogeography Analyses"
...
source("~/Dropbox/Research/Brilliant_hummingbird_biogeography/.extractpos.R")
source("~/Dropbox/Research/Brilliant_hummingbird_biogeography/lets.prabs.R")
source("~/Dropbox/Research/Brilliant_hummingbird_biogeography/removeCells.R")
source("~/Dropbox/Research/Brilliant_hummingbird_biogeography/removeSp.R")
source("~/Dropbox/Research/Brilliant_hummingbird_biogeography/get_node_range_BioGeoBEARS_nonull.R")
...
brilliant_range_chosen <-read.csv("ranges/Brilliant_Range_Wrangler.csv", header=T, row.names=1)
Brill_PA_Mat <- read.table("data/Brill_PA_Mat_.txt", sep="\t")
names(Brill_PA_Mat) <- gsub("\\.", "_", names(Brill_PA_Mat))

keep <- colnames(Brill_PA_Mat)
keep <- keep[3:47]
keep  <- gsub(" ", "_", keep, fixed=T)
...
brilliant_tree$tip.label<-gsub("Oreotroch_estella", "Oreotrochilus_estella", brilliant_tree$tip.label)

brilliant_tree <- keep.tip(brilliant_tree, keep)
...
#We need to change column names (Latitude & Longitude) for our Brilliant data
colnames(Brill_PA_Mat)[colnames(Brill_PA_Mat) == "Longitude(x)"] <- "LONG"
colnames(Brill_PA_Mat)[colnames(Brill_PA_Mat) == "Latitude(y)"] <- "LAT"

Brill_PA_Mat <-  as.data.frame(Brill_PA_Mat)

site_xy.br <- Brill_PA_Mat %>% 
  dplyr::select(LONG, LAT) 
brill_pa <- Brill_PA_Mat %>% 
  dplyr::select(-LONG, -LAT)
```
...
spp_in_tree <- names(brill_pa) %in% brilliant_tree$tip.label
brill_pa_tree <- brill_pa[, spp_in_tree]
```
...
richness <- rowSums(brill_pa_tree)
map_richness <- 
dplyr::bind_cols(site_xy.br, richness = richness) %>% 
  ggplot2::ggplot() + 
  ggplot2::geom_raster(ggplot2::aes(x = LONG, y = LAT, fill = richness)) + 
  rcartocolor::scale_fill_carto_c(name = "Richness", type = "quantitative", palette = "SunsetDark") +
  ggplot2::geom_sf(data = coastline) +
...
```{r outFig1, echo=FALSE, eval=TRUE, out.width="70%", fig.cap="Figure 1 - Species richness for the brilliant hummingbird clade in South America"}
map_richness
```
...
regions <- 
  ...
  comm = brill_pa_tree,
  phy = brilliant_tree,
  max.n.clust=9 #sets maximum number of clusters to try
  )
site_region <- regions$cluster_evoregions

save(regions, file="data/regions_12spp.Rdata")
```
...
```{r outFig2, echo=FALSE,eval=TRUE,out.width="70%",fig.cap="Figure 2 - Evoregions for Brilliant communities"}
map_evoregion
```
...
sites <- dplyr::bind_cols(site_xy.br, site_region =  site_region, afi)
```
...
```{r outFig3, echo=FALSE,eval=TRUE,out.width="70%",fig.cap="Figure 3 - Evoregion and afilliation for communities of brilliant hummingbirds"}
map_joint_evoregion_afilliation
```
...
a_region <- Herodotools::get_region_occ(comm = brill_pa_tree, site.region = site_region)
```
...
Herodotools::get_tipranges_to_BioGeoBEARS(
  ...
  filename = here::here("ranges", "geo_brilliant_5_areas.txt"),
  areanames = NULL
  )
```
...
load("~/Dropbox/Research/Brilliant_hummingbird_biogeography/data/resDEC_brilliant12.RData")
```
...
# getting the ancestral range area for each node #NOT SURE WHAT HAPPENED ABOVE BUT HAD TO TRIM TREE AGAIN
node_area <- get_node_range_BioGeoBEARS(
    BioGeoBEARS.data = resDEC,
    phyllip.file = here::here("ranges", "geo_brilliant_5_areas.txt"),
    tree=brilliant_tree,
    max.range.size = 4 
  )
# calculating age arrival 
age_comm <- Herodotools::calc_age_arrival(W = brill_pa_tree, 
                        tree = brilliant_tree, 
...
sites <- dplyr::bind_cols(site_xy.br, site_region =  site_region, age = age_comm$mean_age_per_assemblage)
map_age <- 
  sites %>% 
  ggplot() + 
...
brilliant_diversification <- 
Herodotools::calc_insitu_diversification(W = brill_pa_tree,
                   tree = brilliant_tree, 
                   ancestral.area = node_area, 
                   biogeo = biogeo_area, 
...
sites <- dplyr::bind_cols(site_xy.br,
                   site_region =  site_region,
                   age = age_comm$mean_age_per_assemblage,
                   diversification_model_based = brilliant_diversification$insitu_Jetz_harmonic_mean_site,
                   diversification = brilliant_diversification$Jetz_harmonic_mean_site)
map_diversification <- 
   sites %>% 
   ggplot2::ggplot() + 
...
```{r outFig5, echo=FALSE,eval=TRUE,out.width="70%",fig.cap="Figure 5 - Diversification rates (DR - A) and in-situ diversification (in-situ diversification - B) for brilliant assemblages"}
...
brilliant_dispersal <- 
Herodotools::calc_dispersal_from(W = brill_pa_tree,
               tree = brilliant_tree, 
               ancestral.area = node_area, 
               biogeo = biogeo_area)
```
...
sites <- dplyr::bind_cols(site_xy.br,
                   ...
                   diversification = brilliant_diversification$Jetz_harmonic_mean_site,
                   diversification_model_based = brilliant_diversification$insitu_Jetz_harmonic_mean_site,
                   dispersal.D = brilliant_dispersal[,1], 
                   dispersal.A = brilliant_dispersal[, 2],
                   dispersal.E = brilliant_dispersal[, 3],
                   dispersal.BD = brilliant_dispersal[, 4],
                   dispersal.B = brilliant_dispersal[, 5])
map_dispersal_E <- 
   sites %>% 
   ggplot2::ggplot() + 
   ggplot2::geom_raster(ggplot2::aes(x = LONG, y = LAT, fill = dispersal.E)) + 
      rcartocolor::scale_fill_carto_c(
      type = "quantitative", palette = "SunsetDark", na.value = "white", limits = c(0,1)
     ) +
   ggplot2::geom_sf(data = coastline, size = 0.4) +
   ggplot2::coord_sf(xlim = map_limits$x, ylim = map_limits$y) +
   ggplot2::ggtitle("From E") + 
   ggplot2::theme_bw() +
...
map_dispersal_BD <- 
   sites %>% 
   ggplot2::ggplot() + 
   ggplot2::geom_raster(ggplot2::aes(x = LONG, y = LAT, fill = dispersal.BD)) +
   rcartocolor::scale_fill_carto_c(
     type = "quantitative", palette = "SunsetDark", na.value = "white", limits = c(0,1)
     ) +
   ggplot2::geom_sf(data = coastline, size = 0.4) +
   ggplot2::coord_sf(xlim = map_limits$x, ylim = map_limits$y) +
   ggplot2::ggtitle("From BD") + 
   ggplot2::theme_bw() +
...
leg <- cowplot::get_legend(map_dispersal_BD)
plot1 <- cowplot::plot_grid(map_dispersal_A + theme(legend.position = "none"), 
                   map_dispersal_BD + theme(legend.position = "none"), 
                   map_dispersal_E + theme(legend.position = "none"), leg,
...
brillPD_PE_insitu <- calc_insitu_metrics(W = brill_pa_tree,
                  tree = brilliant_tree, 
                  ancestral.area = node_area, 
                  biogeo = biogeo_area, 
                  PD = TRUE, 
...
sites <- dplyr::bind_cols(site_xy.br,
                   ...
                   age = age_comm$mean_age_per_assemblage,
                   diversification = brilliant_diversification$Jetz_harmonic_mean_site,
                   PE = brillPD_PE_insitu$PE, 
                   PEinsitu = brillPD_PE_insitu$PEinsitu,
                   PD = brillPD_PE_insitu$PD,
                   PDinsitu = brillPD_PE_insitu$PDinsitu)
map_PE <- 
   sites %>% 
   ggplot2::ggplot() + 
...
This is a package that `Herodotools` uses to look at alpha, beta, diversity and generate phyloregions that may be useful in BIOGEOBEARS

```{r}
library(phyloregion)


pol <- polys2comm(dat = brill.all, species = "binomial", trace=0)
head(pol[[1]][1:5, 1:5])
