...
filename <- "Brilliants_no_out"
#name of the output directory	
dir_name <- "BGB_results"
...
posterior_trees_fn <- "trees/Ster_Hylo_P11.trees"
...
geogfn = "ranges/Brilliant_ranges_all_v1.txt"
...
out.tips <- c("Caprimul_longirostrisB32361", 
              "Streptop_zonaris_NMNHb13064", 
              "Chaetura_pelagica3346", 
              "Topaza_pella_35362", 
              "Florisuga_mellivora9646", 
              "Phaethornis_guy28342", 
              "Glaucis_hirsuta28468", 
              "Eutoxeres_condamini8094", 
              "Anthrac_nigricollis9652", 
              "Colibri_coruscans6310", 
              "Doryfera_johannae1728", 
              "Atthis_heloisafmnh343218", 
              "Calypte_anna24864",
              "Selasphorus_rufus19586",
              "Metallura_phoebe103876",
              "Aglaiocercus_coelestis7813",
              "Oreotroch_estella103835",
              "Eugenes_fulgens9964",
              "Lampornis_clemenciae10119",
              "Heliomaster_longirost18268",
              "Thalurania_furcata2807",
              "Campylopt_hemileucurus26474",
              "Trochilus_polytmuscjc25",
              "Patagona_gigas6303",
              "Hylonympha_macroHM3",
              "Hylonympha_macroHM2",
              "Sternoclyta_cyanoIC984",
              "Sternoclyta_cyanoJM289")

outgroup_taxa <- out.tips
...
num_states <- numstates_from_numareas(numareas=num_areas, maxareas=max_range_size, include_null_range=FALSE)
...
colors_list_for_states = get_colors_for_states_list_0based(areanames=areas, states_list_0based=states_list_0based, max_range_size=max_range_size, plot_null_range=FALSE)		
...
if (DIVALIKE == TRUE & BAYAREALIKE ==TRUE) {
  ...
  if (time_slice_control<tMax_allreps) {
    ...
    for (r in 1:n_trees) {
      ...
      #not working for me try this
      
      first_node <-getMRCA(tr, ingroup$tip.label)
    ...
    if (plot_LTT) {
      ...
      # plots graph
      while (!is.null(dev.list())) {dev.off()} #cleans plotting area
      pdf(file=paste(dir_name, "/Average_LTT_by_area.pdf", sep =""))				
